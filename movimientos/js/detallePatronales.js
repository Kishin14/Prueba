...
function saveDetalleSolicitud(obj){
	...
	if(validaRequeridosDetalle(obj,row)){
	  ...
	  var  detalle_liquidacion_patronal_id	= $(Fila).find("input[name=detalle_liquidacion_patronal_id]").val();
	...
	if( $('#actualizar',parent.document).length > 0 ){
		...
		var QueryString = "ACTIONCONTROLER=onclickUpdate&detalle_liquidacion_patronal_id="+detalle_liquidacion_patronal_id+"&observacion="+observacion;
		
		$.ajax({
			url        : "DetallePatronalesClass.php",
			data       : QueryString,
			beforeSend : function(){
			  setMessageWaiting();
			},
