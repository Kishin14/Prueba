...
function saveDetalleSolicitud(obj){
	...
	if(validaRequeridosDetalle(obj,row)){
	  ...
	  var  detalle_liquidacion_provision_id	= $(Fila).find("input[name=detalle_liquidacion_provision_id]").val();
	...
	if( $('#actualizar',parent.document).length > 0 ){
		...
		var QueryString = "ACTIONCONTROLER=onclickUpdate&detalle_liquidacion_provision_id="+detalle_liquidacion_provision_id+"&observacion="+observacion;
		
		$.ajax({
			url        : "DetalleProvisionesClass.php",
			data       : QueryString,
			beforeSend : function(){
			  setMessageWaiting();
			},
