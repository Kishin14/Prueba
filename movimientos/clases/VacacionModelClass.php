...
final class VacacionModel extends Db{
  ...
  public function Save($Campos,$oficina_id,$Conex){	
		...
		(liquidacion_vacaciones_id,contrato_id,fecha_liquidacion,fecha_dis_inicio,fecha_dis_final,fecha_reintegro,dias,valor,observaciones,concepto)
		VALUES
		($liquidacion_vacaciones_id,$contrato_id,$fecha_liquidacion,$fecha_dis_inicio,$fecha_dis_final,$fecha_reintegro,$dias,$valor,$observaciones,$concepto)";
		 ...
		 $item				= split(',',$detalle_liquidacion);
		  ...
		  foreach($item as $item_id){
			if($item_id!=''){
				...
				$insert_detalles = "INSERT INTO detalle_liquidacion_vacaciones (liquidacion_vacaciones_id,periodo_inicio,periodo_fin,dias_ganados,dias_disfrutados) 						                                    VALUES
								    ($liquidacion_vacaciones_id,$fecha_inicial_periodo,$fecha_final_periodo,$dias_ganados,$dias_disfrutados)";
		...
		if($valor_consolidado>0){
			...
			$valor_ingresado   = $valor_consolidado; 
		...
		VALUES
		($liquidacion_vacaciones_id,$puc_consolidado_vacaciones,$tercero_id,$numero_identificacion,$digito_verificacion,$centro_costo_consolidado,IF($centro_costo_consolidado>0,(SELECT codigo FROM centro_de_costo WHERE centro_de_costo_id = $centro_costo_consolidado),'NULL'),0,'NULL','NULL',$observaciones,$valor_consolidado,0,0,0)";
		...
		VALUES
		($liquidacion_vacaciones_id,$puc_provision_vacaciones,$tercero_id,$numero_identificacion,$digito_verificacion,$centro_costo_provision,IF($centro_costo_provision>0,(SELECT codigo FROM centro_de_costo WHERE centro_de_costo_id = $centro_costo_provision),'NULL'),0,'NULL','NULL',$observaciones,$valor_provision,0,0,0)";
		...
		VALUES
		($liquidacion_vacaciones_id,$puc_diferencia,$tercero_id,$numero_identificacion,$digito_verificacion,$centro_costo_provision,IF($centro_costo_provision>0,(SELECT codigo FROM centro_de_costo WHERE centro_de_costo_id = $centro_costo_provision),'NULL'),0,'NULL','NULL',$observaciones,$diferencia,0,0,0)";
		...
		VALUES
		($liquidacion_vacaciones_id,$puc_salud,$tercero_id,$numero_identificacion,$digito_verificacion,$centro_costo_provision,IF($centro_costo_provision>0,(SELECT codigo FROM centro_de_costo WHERE centro_de_costo_id = $centro_costo_provision),'NULL'),0,'NULL','NULL',$observaciones,0,$descuento_salud,0,0)";
		$this -> query($insert_desc_salud,$Conex,true);
		//exit($insert_desc_salud);
		$insert_desc_pension ="INSERT INTO detalle_vacaciones_puc (liquidacion_vacaciones_id,puc_id,tercero_id,numero_identificacion,digito_verificacion,centro_de_costo_id,codigo_centro_costo,base_vacaciones,porcentaje_vacaciones,formula_vacaciones,desc_vacaciones,deb_item_vacaciones,cre_item_vacaciones,valor_liquida,contrapartida)
		VALUES
		($liquidacion_vacaciones_id,$puc_pension,$tercero_id,$numero_identificacion,$digito_verificacion,$centro_costo_provision,IF($centro_costo_provision>0,(SELECT codigo FROM centro_de_costo WHERE centro_de_costo_id = $centro_costo_provision),'NULL'),0,'NULL','NULL',$observaciones,0,$descuento_pension,0,0)";
		$this -> query($insert_desc_pension,$Conex,true);
		
		
		$insert_det_puc_contra ="INSERT INTO detalle_vacaciones_puc (liquidacion_vacaciones_id,puc_id,tercero_id,numero_identificacion,digito_verificacion,centro_de_costo_id,codigo_centro_costo,base_vacaciones,porcentaje_vacaciones,formula_vacaciones,desc_vacaciones,deb_item_vacaciones,cre_item_vacaciones,valor_liquida,contrapartida)
		VALUES
		($liquidacion_vacaciones_id,$puc_contrapartida,$tercero_id,$numero_identificacion,$digito_verificacion,$centro_costo_provision,IF($centro_costo_provision>0,(SELECT codigo FROM centro_de_costo WHERE centro_de_costo_id = $centro_costo_provision),'NULL'),0,'NULL','NULL',$observaciones,0,$valor_pagar,0,0)";
			...
			$select="SELECT c.contrato_id FROM contrato c WHERE c.estado='A'";
				...
				foreach($result as $resultado){
					...
					$contrato_id = $resultado[contrato_id];
						...
						$select_contrato = "SELECT c.contrato_id,(SELECT e.tercero_id FROM empleado e WHERE e.empleado_id=c.empleado_id)as tercero_id,c.area_laboral,c.sueldo_base,c.fecha_inicio, DATEDIFF(CURDATE(),c.fecha_inicio) as dias_trabajados FROM contrato c WHERE c.contrato_id=$contrato_id AND estado='A' ";
						$result_contrato = $this -> DbFetchAll($select_contrato,$Conex); 
						$contrato_id	 = $result_contrato[0]['contrato_id'];
						$tercero_id	 = $result_contrato[0]['tercero_id'];
						$area_laboral	 = $result_contrato[0]['area_laboral'];
  ...
  public function Update($Campos,$Conex){	
	...
	$fecha_liquidacion		= $this -> requestDataForQuery('fecha_liquidacion','date');
	$empleado_id			= $this -> requestDataForQuery('empleado_id','integer');
	$valor					= $this -> requestDataForQuery('$valor','text');
	$concepto				= $this -> requestDataForQuery('concepto','text');
	...
	$estado			= $this -> requestDataForQuery('estado','text');
	  ...
	  if($liquidacion_vacaciones_id == 'NULL'){
		...
		$update= "UPDATE liquidacion_vacaciones 
		          SET fecha_liquidacion = $fecha_liquidacion,liquidacion_vacaciones_id = $liquidacion_vacaciones_id,contrato_id = $contrato_id ,valor = $valor, concepto = $concepto,dias = $dias,fecha_dis_inicio = $fecha_dis_inicio ,fecha_dis_final = $fecha_dis_final ,fecha_reintegro = $fecha_reintegro, observaciones = $observaciones,estado = $estado
 ...
 public function getContabilizarReg($liquidacion_vacaciones_id,$empresa_id,$oficina_id,$usuario_id,$mesContable,$periodoContable,$Conex){
	 
	$this -> Begin($Conex);
		
		$select 	= "SELECT l.*,(SELECT e.tercero_id FROM empleado e, contrato c WHERE e.empleado_id=c.empleado_id AND c.contrato_id=l.contrato_id)as tercero_id FROM liquidacion_vacaciones l WHERE l.liquidacion_vacaciones_id=$liquidacion_vacaciones_id";	
		$result 	= $this -> DbFetchAll($select,$Conex,true); 
		...
		if($mes_contable_id>0 && $periodo_contable_id>0){
			...
			$select_item      = "SELECT detalle_vacaciones_puc_id  FROM  detalle_vacaciones_puc WHERE liquidacion_vacaciones_id=$liquidacion_vacaciones_id";
    ...
    public function getTotalDebitoCredito($liquidacion_vacaciones_id,$Conex){
	  ...
	  $select = "SELECT SUM(deb_item_vacaciones) AS debito,SUM(cre_item_vacaciones) AS credito FROM detalle_vacaciones_puc   WHERE liquidacion_vacaciones_id=$liquidacion_vacaciones_id";
      $result = $this -> DbFetchAll($select,$Conex,true);
