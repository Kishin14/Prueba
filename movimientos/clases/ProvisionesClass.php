...
final class Provisiones extends Controler{
  ...
  protected function setCampos(){  
	...
	$this -> Campos[liquidacion_provision_id] = array(
		...
		datatype=>array(
			...
			length	=>'11'),
		transaction=>array(
			table	=>array('liquidacion_provision'),
	...
	//BUSQUEDA
   	$this -> Campos[busqueda] = array(
		...
		//tabindex=> '1',
		suggest=>array(
			name	=>'busca_provision',
			setId	=>'liquidacion_provision_id',
  ...
  public function Main(){
	...
	$Layout -> SetGridProvisiones($Attributes,$Titles,$Cols,$Model -> getQueryProvisionesGrid());
	$Layout -> RenderMain();
  }
  ...
  protected function onclickValidateRow(){
    ...
    $Data = new ValidateRow($this -> getConex(),"liquidacion_provision",$this ->Campos);
  ...
  protected function onclickCancellation(){
	 ...
	 $estado=$Model -> comprobar_estado($liquidacion_provision_id,$this -> getConex());
	 ...
	 $Model -> cancellation($liquidacion_provision_id,$estado[0]['encabezado_registro_id'],$causal_anulacion_id,$observacion_anulacion,$usuario_anulo_id,$this -> getConex());
  ...
  protected function getTotalDebitoCredito(){
  	...
  	require_once("ProvisionesModelClass.php");
    $Model = new ProvisionesModel();
	$liquidacion_provision_id = $_REQUEST['liquidacion_provision_id'];
	$estado=$Model -> comprobar_estado($liquidacion_provision_id,$this -> getConex());
	$data = $Model -> getTotalDebitoCredito($liquidacion_provision_id,$this -> getConex());
  ...
  protected function getContabilizar(){
    ...
    if($mesContable && $periodoContable){
		$return=$Model -> getContabilizarReg($liquidacion_provision_id,$empresa_id,$oficina_id,$usuario_id,$mesContable,$periodoContable,$this -> getConex());//aca
  ...
  protected function onclickFind(){
    ...
    $Model     		= new ProvisionesModel();
    $Liquidacion 	= $_REQUEST['liquidacion_provision_id'];
